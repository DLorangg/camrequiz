...
const file = 'hooks/useCamreQuiz.ts';
let content = fs.readFileSync(file, 'utf8').replace(/\r\n/g, '\n');

// 1. Insert Effects after states (around availableCharactersForCrown)
const insertMarker = '  const [availableCharactersForCrown, setAvailableCharactersForCrown] = useState<string[]>([])\n';
...
content = content.replace(insertMarker, insertMarker + newEffects);
...
if (startIdxSpin !== -1 && endIdxSpin !== -1) {
  const newSpinWheel = `  const spinWheel = () => {
    ...
    if (availableCategoriesWithQuestions.length === 0) {
      console.log("No hay más preguntas disponibles en TODAS las categorías seleccionadas.")
      return
    }
...
`;
  ...
  console.log("Could not find spinWheel");
}
...
`;
  ...
  console.log("Could not find resetGame");
}
